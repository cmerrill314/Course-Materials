...
{
	/*write this*/
	assert(val != 0); //val cannot be null

	int compareValue;

	//BASE CASE: If the root node is empty, create a new node with value val
	if (cur == 0)
	{
		//Create an new node 
		struct Node *node;
		...
		//Update the node value and pointers
		node->val = val;
		node->right = 0;
		node->left = 0;
	...
	//If the root node isn't empty, use the compare function to determine if 
	//val should move to the left or to the right
	compareValue = compare(val, cur->val);
...
{
	/*write this*/
	assert(tree != 0); //tree cannot be null
	assert(val != 0);  //val cannot be null
	...
	while (cur != 0)
	{
		//use the compare function to determine which node to evaluate next
		compareValue = compare(val, cur->val);
		
		//if the current value equals val, the element was found
		if (compareValue == 0)
			return 1;
		//if val is less then the current value, check the left node
		else if (compareValue == -1)
			cur = cur->left;
		//if val is greater then the current value, check the right node
		else
			cur = cur->right;
	}
...
{
	/*write this*/
	assert(cur != 0); //cur annot be null

	//if the current value is null, the tree is empty
	if (cur == 0)
		return 0;
	else
	{
		//iterate through the tree to find the leftmost value
		while (cur->left != 0)
			cur = cur->left;
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	//assure cur is not null
	if (cur != 0) 
	{
		if (cur->left == 0) 
		{
			//if the left child is null, return the right child of cur and free cur
			rightChild = cur->right;
			free(cur);
			return rightChild;
		}
		else 
			//if the left child is not null, recursively check the next element and return cur
...
{
	/*write this*/
	assert(cur != 0); //cur cannot be null
	assert(val != 0); //val cannot be null
	struct Node *node;

	//Compare val to the current value using compare()
	int compareValue = compare(val, cur->val);

	//if val is less than the current value, move to the left child of cur
	if (compareValue == -1)         
		cur->left = _removeNode(cur->left, val);
	//if val is greater than the current value, move to the right child of cur
	else if (compareValue == 1)
		cur->right = _removeNode(cur->right, val);
	//if val is equal to the current value, remove it
	else
	{
		//Case 1: val has a right child and a left child
		if (cur->right && cur->left)
		{
			//set node equal to the smallest value in cur's right subtree
			node = _leftMost(cur->right);
			//replace the current node with the smallest value in the right subtree
			cur->val = node->val;
			//remove the duplicate value
			cur->right = _removeNode(cur->right, node->val);
		}
		//Case 2: val has one or no children
		else
		{
			node = cur;
			//if the node only has a right child, set it equal to the right child
			if (cur->left == 0)
				cur = cur->right;
			//if the node only has a left child, set it equal to the left child
			else if (cur->right == 0)
				cur = cur->left;
			free(node);
		}
