...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//Create a stack structure to search the graph
	Deque *stack = dequeNew();

	//Reset "isVisited" for each vertex
	clearVisited(graph);
	...
	while (dequeIsEmpty(stack) != 1)
	{
		//Start with the vertex at the back of the stack
		Vertex *curVertex = dequeBack(stack);

		//Remove the back value for the next iteration
		dequePopBack(stack);

		//If the current vertex is the destination vertex, then a path was found
		...
		//Add the current vertex' neighbors to the stack so they can be searched
		for (int i = 0; i < curVertex->numNeighbors; i++)
		{
			if (curVertex->neighbors[i]->isVisited == 0)
			{
				dequePushBack(stack, curVertex->neighbors[i]);
				curVertex->neighbors[i]->isVisited = 1;
			}
		}
		//Mark the current vertex as "visited"
		curVertex->isVisited = 1;
	}
	//Free the stack when the search is complete
	dequeDelete(stack);
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//Create a queue structure to search the graph
	Deque *queue= dequeNew();

	//Reset "isVisited" for each vertex
	clearVisited(graph);

	//Add the source vertex to the back of the queue
	dequePushBack(queue, source);

	//Traverse each vertex in the graph
	while (dequeIsEmpty(queue) != 1)
	{
		//Start with the vertex at the front of the queue
		Vertex *curVertex = dequeFront(queue);

		//Remove the front value for the next iteration
		dequePopFront(queue);

		//If the current vertex is the destination vertex, then a path was found
		if (curVertex == destination)
		{
			//Free the queue and return 1
			dequeDelete(queue);
			return 1;
		}
		
		//Add the current vertex' neighbors to the queue so they can be searched
		for (int i = 0; i < curVertex->numNeighbors; i++)
		{
			if (curVertex->neighbors[i]->isVisited == 0)
			{
				dequePushBack(queue, curVertex->neighbors[i]);
				curVertex->neighbors[i]->isVisited = 1;
			}
		}
		//Mark the current vertex as "visited"
	...
	//Free the queue when the search is complete
	dequeDelete(queue);
    return 0;
}
