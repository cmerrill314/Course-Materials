...
Program 4                                        *
                                                 *
This file handles encryption on the server side. *
*************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		int plainSize, plainLength, keySize, keyLength, count, rec, plainVal, keyVal, encryptedVal;
		char buffer[1024];
		char* plainText;
		char* keyText;
		...
		//Child Process
			...
			//Confirm that otp_enc is trying to connect
			receiveText = htonl(1);
			if (send(establishedConnectionFD, &receiveText, sizeof(receiveText), 0) == -1) error("ERROR: Failed to receive text from client\n");
			
			//Receive plain text and key text size
			if (recv(establishedConnectionFD, &plainSize, sizeof(plainSize), 0) == -1) error("ERROR: Failed to receive plain text size from client\n");
			else if (plainSize == 0) error("ERROR: Plain text size is 0\n");
			...
			//Set the plain text and key text length
			plainLength = ntohl(plainSize);
			keyLength = ntohl(keySize);
			
			//RECIEVE PLAIN TEXT
			plainText = malloc(sizeof(char) * plainLength);
			memset(plainText, '\0', plainLength);
			count = 0;
			...
			while (count <= plainLength) {
				...
				if (rec == -1) {error("ERROR: Failed to receive plain text\n"); break;}
				else if (rec == 0) {
					//Break if there is still more to receive
					if (count < plainLength) break;
				}
			...
			//Encrypt each character of plain text
			for (i = 0; i < plainLength - 1; i++) {
				//Transform plain text character to int
				if (plainText[i] != ' ') plainVal = plainText[i] - 65;
				else plainVal = 26; //space case
				
				//Transform key text character to int
				if (keyText[i] != ' ') keyVal = keyText[i] - 65;
				else keyVal = 26; //space case
				
				//Encrypted value is sum of plain and key values
				encryptedVal = plainVal + keyVal;
				//If encrypted value is 27 or over, subtract 27
				if (encryptedVal >= 27) encryptedVal -= 27;
				
				//Replace each plain text character with the encrypted character
				if (encryptedVal != 26) plainText[i] = 'A' + (char)encryptedVal;
				else plainText[i] = ' '; //space case
			}
			...
			while (count <= plainLength) {
				char encryptedString[1024];
				
				//Copy plain text to encrypted string
				strncpy(encryptedString, &plainText[count], 1023);
				encryptedString[1024] = '\0'; //add null terminator
				if (send(establishedConnectionFD, &encryptedString, 1024, 0) == -1) error("Error: Unable to send encrypted string\n");
